...
rate_limit_or_fail('password_otp_request', 5, 30);
...
function get_admin_password_hash(PDO $pdo, int $adminId): ?string
{
    $stmt = $pdo->prepare('SELECT password_hash FROM admin_users WHERE id = :id AND is_active = 1 LIMIT 1');
    $stmt->execute([':id' => $adminId]);
    $hash = $stmt->fetchColumn();
    return is_string($hash) && $hash !== '' ? $hash : null;
}

function cleanup_admin_password_otps(PDO $pdo, int $adminId): void
{
    $stmt = $pdo->prepare(
        'DELETE FROM admin_password_otps
         WHERE admin_user_id = :admin_user_id
           AND (used_at IS NOT NULL OR expires_at_epoch < :expired_before)'
    );
    $stmt->execute([
        ':admin_user_id' => $adminId,
        ':expired_before' => time() - 300,
    ]);
}
...
try {
    ...
    if (!$sent) {
        ...
        json_response(false, 'Unable to send OTP right now.', 500);
    }
    ...
    json_response(true, 'OTP sent to admin email.', 200, [
        'data' => [
            'request_token' => $requestToken,
            'expires_in' => 60,
    ...
    error_log('Request admin password OTP error: ' . $e->getMessage());
    json_response(false, 'Unable to send OTP right now.', 500);
}
