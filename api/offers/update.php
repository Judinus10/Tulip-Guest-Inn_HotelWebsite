...
try {
    ...
    $existing = offer_find($pdo, $id);
    if (!$existing) {
        offer_json(['success' => false, 'message' => 'Offer not found.'], 404);
    }
    ...
    $uploadedImage = offer_upload_image('image');
    $imagePath = $uploadedImage ?? offer_storable_image_path($_POST['image_path'] ?? '');
    ...
    $stmt = $pdo->prepare(
        'UPDATE offers
         SET title = :title,
             ...
             booking_scope = :booking_scope, minimum_nights = :minimum_nights, minimum_rooms = :minimum_rooms,
             minimum_guests = :minimum_guests, priority = :priority, automatic_apply = :automatic_apply
         WHERE id = :id'
    );
